...
export const userUpdate = (fastify: ProtectedFastify) => {
  return async (request: ProtectedRequest, reply: FastifyReply) => {
    ...
    const { oldValues, newValues } = (request.body as Static<typeof RequestBodyUserUpdateSchema>).payload.data // guaranteed to exist
    ...
    if (receivedFields.length === 0) throw new ErrorDefensiveGuardBreach("ERR_NO_UPDATE_FIELD_PROVIDED", "Update attempt received with no fields to update.")
    ...
    const values = [...receivedFields.map((field) => newValues[field as keyof typeof newValues]), `${id}`]
