 ...
 */
export const userRefreshAccessTokenProxyAdapter = (fastify: FastifyInstance) => {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    // 1. Extract the browser ID (`bid`), the internal destination path (`proxyPath`), and the payload.
    const { bid, proxyPath, payload } = request.body as Static<typeof UserRefreshAccessTokenProxyAdapterSchema> // Guaranteed to exist
    ...
    console.log(
      "userRefreshAccessTokenProxyAdapter---rawReply",
      rawReply,
      "🌻json() rawReply---->",
      rawReply.json(),
      '[overview, ...(rawReply.json().sideEffects?.devNotes ? [rawReply.json().sideEffects.devNotes] : [""])]',
      [overview, ...(rawReply.json().sideEffects?.devNotes ? [rawReply.json().sideEffects.devNotes] : [""])]
    )
