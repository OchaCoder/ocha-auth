...
export const userResetPasswordPreEmail = (fastify: FastifyInstance) => {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    // 1. Extract email from request body
    const { email } = (request.body as UserResetPasswordSchemaPreEmail).payload.data
    ...
    if (validatedData === null) return reply.status(400).send({ success: false, code: "ERR_PASS_RESET_REQUEST_W_UNREGISTERED_EMAIL" })
