...
export const userSignin = (fastify: FastifyInstance) => {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    const { email, password, bid } = (request.body as UserSignInPayload).payload.data
    ...
    if (validatedData === null) return reply.status(403).send({ success: false, code: "ERR_UNREGISTERED_SIGNIN_REQUEST" })
    ...
    await runRedis.fireAndForget(
      ...
      async () => {
        ...
        if (length >= 10) await fastify.redis.zpopmin(`user:${validatedData.id}:browsers:`)
