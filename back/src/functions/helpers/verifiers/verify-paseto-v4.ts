 ...
 */
export const verifyPasetoV4 = async (reply: FastifyReply, v4Token: string, key: string, userIdentifier: string, tokenName: "at" | "rt" | "rpt" | "rptUserObj") => {
  try {
      ...
      case "ERR_PASETO_CLAIM_INVALID": // One of the claims (e.g. exp) is invalid
        ...
        // the frontend cookie's misconfiguration cannot be ruled out completely.
          ...
          throw new ErrorSuspiciousActivity(
            "ERR_AT_COOKIE_PRESENT_BUT_V4_EXPIRED",
            "Despite present in cookie, access token was expired. Unless the frontend cookie is misconfigured, this may indicate exploratory or tampering.",
        ...
        // Which is why this case triggers an `ErrorDefensiveGuardBreach` instead of `ErrorSuspiciousActivity`.
          ...
          throw new ErrorDefensiveGuardBreach(
            "ERR_RT_COOKIE_PRESENT_BUT_V4_EXPIRED",
            "Despite browser ID was present in cookie and refresh token was available in Redis, refresh token was expired. Possibly bug during token generation",
            { specialJson: JSON.stringify({ v4Token }), debug: { rt: v4Token } }
          )
        } else if (tokenName === "rptUserObj") {
          throw new ErrorDefensiveGuardBreach(
            "ERR_RPT_USER_OBJ_EXPIRED_EARLIER_THAN_RPT_OR_REDIS",
            "This is the user object stored in Redis as V4 string. The Redis and its key which also derives from V4, are all set to expire in staggered timing (0, +10, +15 secs), so this case is normally impossible. Possibly bug during token generation",
            { specialJson: JSON.stringify({ v4Token }), debug: { rt: v4Token } }
          )
        }
