...
export const connectedClients = new Set<{
  // 'send' property takes a function that is a conveninent thin wrapper
  // around NodeJS.WritableStream ('reply.raw').
  send: (data: any) => void
}>()

// Broadcast the current health status to all connected clients.
export const broadcastCurrentHealth = (fastify: FastifyInstance) => {
...
export const sseHealth = (fastify: FastifyInstance) => {
  const latestHealth = {
