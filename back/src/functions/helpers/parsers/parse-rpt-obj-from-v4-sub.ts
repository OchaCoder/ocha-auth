 ...
 */
export const parseRptObjFromV4Sub = (sub: string): { rpt: string; id: number; email: string } => {
  ...
  if (!("rpt" in parsedSub) || !("id" in parsedSub) || !("email" in parsedSub)) {
    throw new ErrorDefensiveGuardBreach(
      "ERR_V4_SUB_MISSING_PROPERTY",
      "The `sub` from the decrypted Paseto v4 object should have all `rpt`,`id`, and `email` properties. Likely a bug during token generation.",
      {
        specialJson: sub,
        debug: parsedSub,
      }
  ...
  if (typeof parsedSub.rpt !== "string" || typeof parsedSub.id !== "number" || !ValidatorEmail.Check(parsedSub.email)) {
    throw new ErrorDefensiveGuardBreach(
      "ERR_V4_SUB_INVALID_PROPERTY_VALUE",
      "The `sub` from the decrypted Paseto v4 object contains correct set of properties, but the value(s) found invalid. Likely a bug during token generation.",
      { specialJson: sub, debug: parsedSub }
    )
  }
