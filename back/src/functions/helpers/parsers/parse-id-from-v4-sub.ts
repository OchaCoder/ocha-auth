 ...
 */
export const parseIdFromV4Sub = (sub: string): number => {
  ...
  if (!isNumber(parsedSub.id))
    throw new ErrorDefensiveGuardBreach(
      "ERR_V4_SUB_INVALID_PROPERTY",
      "The `sub` from the decrypted Paseto v4 object contains `id`, but the value is not a number. Likely a bug during token generation.",
