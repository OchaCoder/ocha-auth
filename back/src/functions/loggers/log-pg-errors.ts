...
export const errLogPgFailedHealthCheck = (fastify: FastifyInstance) => {
...
export const errLogPgFailedOnStartUp = (fastify: FastifyInstance, maxRetries: number) => {
  return console.error(`
                ----------------------------------------------------
...
export const errLogPgWrongCredential = (fastify: FastifyInstance) => {
  return console.error(`
                ----------------------------------------------------
                ${chalk.red("💥 [ERROR]".padEnd(24))}${chalk.red(`node-pg (@fastify-postgres)`)}
                ${chalk.hex("#ff5733")("⏱️  [TIMESTAMP]".padEnd(24))} ${new Date().toISOString()}
...
export const errLogPgUserRequestNotFulfilled = (fastify: FastifyInstance, userIdentifier: string | number, userWait: number) => {
  console.error(`
              ----------------------------------------------------
