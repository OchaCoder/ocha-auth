...
import { parseRptFromV4Sub } from "../helpers/parsers/parse-rpt-obj-from-v4-sub.js";
export const resolveRptFromRptV4Websafe = async (reply, rptV4Websafe, userIdentifier) => {
    // 1. 'webSafeBtoA' was used at the encoding stage. `webSafeAtoB(rpt)` is the pairing helper for decoding.
    const expectRptV4 = webSafeAtoB(rptV4Websafe);
    ...
    const validatedRpt = parseRptFromV4Sub(decryptedV4ObjWithSub.sub);
    // 6. Return
    return validatedRpt;
};
