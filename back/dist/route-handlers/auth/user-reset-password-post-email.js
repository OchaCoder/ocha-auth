...
export const userResetPasswordPostEmail = (fastify) => {
    return async (request, reply) => {
        const { password, rpt } = request.body.payload.data;
        ...
        if (!resultBool)
            throw new ErrorSuspiciousActivity("ERR_USER_VANISHED_MID_OPERATION", `User requested a password reset email, then vanished, then continued to update password. Possibly exploratory, tampering, or replay attack.`, {
                identity: `${userId}`,
            });
        // 6. Send reply
        reply.code(200).send({
