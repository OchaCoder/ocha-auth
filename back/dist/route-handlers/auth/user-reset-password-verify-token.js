...
export const userResetPasswordVerifyToken = (fastify) => {
    return async (request, reply) => {
        const { rpt } = request.body.payload.data;
        ...
        if (validatedRedisData === null)
            return reply.status(401).send({ success: false, code: "ERR_RPT_EXPIRED" });
        // 5. Check if the token has been used before.
        if (validatedRedisData.isUsed)
            return reply.status(403).send({ success: false, code: "ERR_RPT_IS_USED" });
