...
export const userDeleteAT = (fastify) => {
    return async (request, reply) => {
        ...
        console.log("DecodedAT:", obj, legitATObj, userId);
        let client;
        try {
            // 1: Create client to connect to the database
            client = await fastify.pg.connect();
            // 2: Check if user's email already exists in the database
