...
import { errLogIoredisFailedHealthCheck } from "../functions/loggers/log-ioredis-error.js";
...
export const healthCheckRedis = fp(async (fastify) => {
    ...
    fastify.addHook("onClose", (instance, done) => {
        if (fastify.healthIdRedis)
            clearTimeout(fastify.healthIdRedis);
     ...
     */
    const healthCheckRedis = (checkType) => {
        ...
        (async () => {
            try {
                ...
                if (checkType === "ROUTINE")
                    interval = routineInterval; // Increase routine interval to reduce unnecessary pings
                ...
                if (fastify.redisAvailable)
                    errLogIoredisFailedHealthCheck(fastify);
